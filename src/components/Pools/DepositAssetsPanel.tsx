...
interface DepositAssetsPanelProps {
  onSelectAsset: (token: `0x${string}`) => void
  className?: string
}

const DepositAssetsPanel = ({
  onSelectAsset,
  className,
}: DepositAssetsPanelProps) => {
  const { balances } = useAccountLendingInfo()
  const { borrowRates, utilizations, decimals, symbols, supportedTokens, weights } =
    useTotalLendingInfo()
  ...
  return (
    <div className={`bg-white dark:bg-[#080811] py-6 ${className}`}>
      <h2 className="text-xl font-bold px-6">Assets to deposit</h2>
      <div className="overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] mt-6">
        <div className="w-full min-w-[600px]">
          ...
          {supportedTokens?.map((token) => (
            <div className="flex items-center  px-6 hover:bg-black/5 dark:hover:bg-white/5 transition-all duration-150 border-b border-b-[#afafaf7a] dark:border-b-[#d2d2d212]">
              ...
              <div className="flex-1 min-w-[80px] sm:min-w-[100px] text-center">
                <span>
                  {formatNumberUnit(
                    Number(
                      formatUnits(
                        balances?.[token.toLowerCase() as `0x${string}`] ?? 0n,
              ...
              <div className="flex items-center justify-end space-x-2 flex-1 min-w-[160px] sm:min-w-[200px] text-center">
                <a
                  onClick={() =>
                    onSelectAsset(token.toLowerCase() as `0x${string}`)
                  }
...
export default DepositAssetsPanel
