...
interface BorrowAssetsPanelProps {
  className?: string
  onSelectAsset: (token: `0x${string}`) => void
}

const BorrowAssetsPanel = ({
  onSelectAsset,
  className,
}: BorrowAssetsPanelProps) => {
  const { totalCollateral, totalBorrowed } = useAccountLendingInfo()
  const { borrowRates, supportedTokens, decimals, symbols } =
    useTotalLendingInfo()
...
export default BorrowAssetsPanel
