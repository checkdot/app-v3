...
const DepositsPanel = ({ onSelectAsset, className }: DepositsPanelProps) => {
  const { deposits } = useAccountLendingInfo()
  const { borrowRates, utilizations, decimals, symbols } = useTotalLendingInfo()
  const prices = usePrices()
  const chainId = useChainId()

  const chain = SUPPORTED_CHAINS.find((chain) => chain.id === chainId)

  const availDeposits = Object.entries(deposits ?? {}).filter(
    ([_, balance]) => balance > 0n
  )

  return (
    <div className={`bg-white dark:bg-[#080811] py-6 ${className}`}>
      <h2 className="text-xl font-bold px-6">Your Deposits</h2>
      {availDeposits.length > 0 ? (
        <div className="overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] mt-6">
          <div className="w-full min-w-[600px]">
            ...
            {availDeposits.map(([token, balance]) => {
              return (
                <div
                  key={token}
        ...
        <div className="text-sm opacity-50 px-6 mt-6">No deposits yet</div>
      )}
    </div>
  )
}
